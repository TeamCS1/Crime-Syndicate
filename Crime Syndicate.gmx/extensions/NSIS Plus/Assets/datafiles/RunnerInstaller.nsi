...
; GMS1 games need MSVC 2008 runtime whereas GMS2 games need MSVC 2015
; IMPORTANT: Remember to copy the MSVC executables to 
;            "C:\Users\<USERNAME>\AppData\Roaming\GameMaker-Studio\makensis\" (GMS1)
; 			 "C:\ProgramData\GameMakerStudio2\Cache\runtimes\runtime-<VERSION>\windows\" (GMS2)
;            so the script can find them and include them in the bundle

; If your game is made using GMS2, then uncomment USING_MSVC2015, and remove USING_MSVC2008
!define USING_MSVC2008 1 
;!define USING_MSVC2015 1 

!define DX_INSTALLER        "dxwebsetup.exe"
!define MSVC2008_INSTALLER  "vcredist_x86.exe"
!define MSVC2015_INSTALLER  "vc_redist.x86.exe"

; Variables to hold Setup errors
...
Section "DirectX Install" SEC_DIRECTX
	...
	SetOutPath "$TEMP"
	File "${MAKENSIS}\${DX_INSTALLER}"
	DetailPrint "Installing Microsoft DirectX®..."
	ExecWait '"$TEMP\${DX_INSTALLER}" /Q' $DirectXSetupError
	DetailPrint "Finished Microsoft DirectX® Setup"
	Delete "$TEMP\${DX_INSTALLER}"
	${If} $DirectXSetupError != "0"
		DetailPrint "Microsoft DirectX® was not installed"
	${Else}
...
; define MSVC installer version and executable
!ifdef USING_MSVC2008
	!define MSVC_VERSION "2008"
	!define MSVC_INSTALLER ${MSVC2008_INSTALLER}
!endif
!ifdef USING_MSVC2015	
	!define MSVC_VERSION "2015"
	!define MSVC_INSTALLER ${MSVC2015_INSTALLER}
!endif

; MSVC installer component
Section "MSVC++ Install" SEC_MSVC

	; Make this component required
	SectionIn RO
	
	SetOutPath "$TEMP"
	File "${MAKENSIS}\${MSVC_INSTALLER}"
	DetailPrint "Installing Microsoft Visual C++ ${MSVC_VERSION} Runtime..."
	
	!ifdef USING_MSVC2008
		ExecWait '"$TEMP\${MSVC_INSTALLER}" /q' $MSVCSetupError
	!endif

	!ifdef USING_MSVC2015
		ExecWait '"$TEMP\${MSVC_INSTALLER}" /quiet /norestart' $MSVCSetupError
	!endif
	
	DetailPrint "Finished Microsoft Visual C++ ${MSVC_VERSION} Runtime Setup"
	Delete "$TEMP\${MSVC_INSTALLER}"
	${If} $MSVCSetupError != "0"
		DetailPrint "Microsoft Visual C++ ${MSVC_VERSION} Runtime was not installed"
	${Else}
		DetailPrint "Microsoft Visual C++ ${MSVC_VERSION} Runtime was successfully installed"		
	${EndIf}
...
LangString DESC_SEC03 ${LANG_ENGLISH} "This will install Microsoft Visual C++ ${MSVC_VERSION} Runtime. Required."
LangString DESC_SEC04 ${LANG_ENGLISH} "This will create a shortcut for ${PRODUCT_NAME} on the Desktop."
LangString DESC_SEC05 ${LANG_ENGLISH} "This will create shortcuts for ${PRODUCT_NAME} on the Start Menu."
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
	...
	!insertmacro MUI_DESCRIPTION_TEXT ${SEC_MSVC} $(DESC_SEC03)
	!insertmacro MUI_DESCRIPTION_TEXT ${SEC_DESKTOP_SHORTCUT} $(DESC_SEC04)
	!insertmacro MUI_DESCRIPTION_TEXT ${SEC_STARTMENU_SHORTCUT} $(DESC_SEC05)
