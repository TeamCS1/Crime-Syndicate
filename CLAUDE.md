...
Exhaustive review of all 278 objects (`objects/*.object.gmx`) and all 75 scripts (`scripts/*.gml`), conducted via 10 parallel deep-dive passes. **Totals: 23 Major, 49 Medium, 78 Minor — 150 findings.** When a bug below is fixed, move its entry to the Completed / Patch Notes section below with a short note on the fix, rather than deleting it.
...
- **[obj_black_market_main_contracts_button.object.gmx, missing click event]** — Has Create/Mouse-Enter/Mouse-Leave/Draw but no eventtype=6 enumb=7 (Left Released) event at all, and no parent object. Hover cursor/highlight work, but clicking does nothing — Contracts List is unreachable.
- **[obj_black_market_main_statistics_button.object.gmx, missing click event]** — Identical defect: no click handler anywhere in the file. Statistics menu is unreachable via this button.
- **[obj_black_market_trips_agents_slot3.object.gmx, Left Release event (eventtype 6, enumb 7), page-2/slot12 branch]** — Copy-paste bug: `else if global.slot12Status && global.moneyCountMillion > 0` is missing the `== "EMPTY"` comparison that every sibling slot object (slot1, slot2, slot4–slot9) has in the equivalent branch. `global.slot12Status` is a string, so using it directly as a boolean operand of `&&` is a GML type-mismatch. Concrete failure: player opens the Trips Agents panel, switches to page 2 (slots 10–18), and clicks slot 3 (which maps to slot 12) any time the first `if` (slot12Status == "EMPTY" && moneyCount > slotPrice) is false — essentially always once slot 12 has been purchased/recruited/equipped. The click handler then evaluates the malformed boolean expression and errors out.
...
- **[obj_createPersonalMedicHealTrigger.object.gmx, Left-Click event (eventtype 6, enumb 7)]** — Paying for a full heal via the "convert a banked million" fallback path never actually deducts money, because the final line overwrites the cost-subtraction result. Concrete scenario: player has `global.moneyCountMillion >= 1` and insufficient regular cash. Code does `global.moneyCount += 1000000; ... tempMoney = global.moneyCount; global.moneyCount = 999999; global.moneyCount -= healthConditionCost; global.healthCount = global.healthCountMax; global.moneyCount = tempMoney + 1;` — that last assignment discards the `999999 - healthConditionCost` result entirely. Net effect: player is healed to full, `moneyCountMillion` is spent, but `moneyCount` ends up as `(original + 1,000,000) + 1` — they gain money instead of paying for the heal. Repeatable indefinitely while `moneyCountMillion >= 1`, i.e. an economy exploit / de facto money duplication + free healing.
- **[obj_londonJobFiveBar.object.gmx, Alarm event "Rating system V2"]** — The rank/rating calculation for London Job Five/Ten reads and writes **Chicago's** globals instead of London's: `global.jobFiveBarChicago`, `global.jobFiveBarRankChicago`, `global.jobTenBarChicago`, `global.jobTenBarRankChicago`. The Draw GUI event correctly draws `global.jobFiveBarLondon`/`global.jobTenBarLondon`, but nothing ever advances `global.jobFiveBarRankLondon`/`global.jobTenBarRankLondon` — London Job Five/Ten rank is permanently stuck at "F" and the bar values grow unbounded. Simultaneously, whenever the player views this screen, if Chicago's job5/10 bar happens to be ≥100 the code silently ranks up Chicago's job and resets Chicago's bar to 0 — corrupting Chicago's save progress from an unrelated London screen.
...
- **[obj_propertiesPurchaseOne.object.gmx, obj_propertiesPurchaseThree.object.gmx, obj_propertiesPurchaseFive.object.gmx — Mouse Left Released (eventtype 6, enumb 7), all 9 property-tier branches]** — The "falls into billion money parameters" `else if` branch converts 1 billion into 1000 million (`global.moneyCountMillion += 1000; global.moneyCountBillion -= 1;`) but never tops up `global.moneyCount` from that converted million before deducting the property cost, unlike the sibling "million" branch which correctly does `global.moneyCount += 1000000;` first. Scenario: player has `moneyCount = 0`, `moneyCountMillion = 0`, `moneyCountBillion >= 1`. Buying Uninspiring Garage ($5000) takes the billion branch, sets `moneyCountMillion = 1000`, grants the property, then runs `global.moneyCount -= 5000`, leaving `moneyCount = -5000`. Negative base cash then breaks every future `moneyCount >=` purchase check. This exact mistake repeats across all 9 tiers in all 3 objects (27 occurrences). `scripts/scr_deduct_money.gml` already implements the correct cascading borrow/normalize logic but isn't used here.
- **[obj_trips_slot3.object.gmx / obj_trips_slot4.object.gmx / obj_trips_slot5.object.gmx, Left Released (eventtype 6 enumb 7)]** — All three slots' "unlock while Locked" purchase logic is copy-pasted verbatim from `obj_trips_slot2` and still charges the slot-2 price ($100,000, or the equivalent net cost via the `moneyCountMillion -= 1; moneyCount += 900000` path) instead of their own advertised price. `obj_trips_loadout_gui` (Create event) sets `global.slot3Cost = "Unlock for: $500,000"`, `global.slot4Cost = "Unlock for: $1.0m"`, `global.slot5Cost = "Unlock for: $2.0m"`, but slot3/4/5's purchase code only ever does `global.moneyCount -= 100000` (net $100k). A player with $100,000–$999,999 can unlock Trip Slot 5 (advertised $2,000,000) for $100,000 — a ~95% discount, a real economy exploit. `obj_trips_slot6/7/8` correctly scale their deduction to their own price ($4.0m/$6.0m/$8.0m), proving 3/4/5 are the copy-paste-drift outliers.
- **[obj_trips_loadout_gui_close_x.object.gmx, Left Released (eventtype 6 enumb 7)]** — The "close" (X) button never destroys `obj_trips_loadout_gui` or its children (unlike `obj_sharedInspectorClassDestroyButton`, which explicitly calls `instance_destroy()`), so clicking Close doesn't appear to close the window. It also contains `if instance_exists(obj_trip_upgrades_button_trigger) { instance_create(x,y,obj_trip_upgrades_button_trigger); }` — inverted from the idiom used everywhere else. Since the original trigger normally still exists, this fires on essentially every click, spawning an ever-growing pile of duplicate trigger instances at the close button's own coordinates (instance leak).
- **[obj_rob_j_store_bar4.object.gmx, obj_rob_small_bank_bar5.object.gmx, Step (eventtype 3 enumb 0)]** — Both are missing a rank guard present in sibling `obj_steal_a_car_bar2`. All three share an unconditional tail block: `if global.jobXBarNewYork >= 98 && <= 99 { scr_JobsRankingDropTables(1,7,7,8); ...; global.jobXBarNewYork++; }`. In `obj_steal_a_car_bar2` this additionally requires `&& ratingListOne == "A"` (resp. `ratingListTwo`), gating the top-tier drop to players actually at A-rank. `obj_rob_j_store_bar4`/`obj_rob_small_bank_bar5` lack that condition, so any time the bar crosses 98–99% at **any** rank (even F), the code fires the same top-tier `(1,7,7,8)` drop table reserved for A→S transitions.
...
- **[obj_createPersonalMedicHealTrigger.object.gmx — see above; also see obj_city_bosses_tier_one/two/three_button.object.gmx, Left-Click event, Chicago block, "Checks what City..." action]** — Code reads `if instance_exists(obj_worldMapChicagoControl)` but then does `with obj_worldMapNewYorkControl { instance_destroy(); }` — destroying the wrong city's controller object, repeated identically in all three tier-button files. Starting a Chicago boss battle never destroys `obj_worldMapChicagoControl`, so the player can still interact with the world map / switch cities mid-Chicago-boss-battle.
- **[scr_CreateOptions.gml, line 36-39]** — `obj_debug_button` is created unconditionally every time the Options menu opens, with no build/debug flag gating it anywhere in the project. This button (see `objects/obj_debug_button.object.gmx`) calls `scr_DebugConsole()` on left-click, a full cheat console exposed to every player from the normal Options menu. Any player can run `/give money custom`, `/give skillpoints custom`, `/add properties max`, `/xplevel`, `/reset`, etc. — giving unlimited money/skillpoints/properties.
- **[scr_LoadGameScript.gml, ~line 233]** — The ini key used to read back the Agency Level Hub is typo'd as `"global.agencyLevelHub"` instead of `"agencyLevelHub"`. `scr_save_script.gml` (line 198) writes the value under the correct key, so this read never matches and always falls back to `0` — the player's Agency Level Hub progression resets on every load despite being saved correctly.
